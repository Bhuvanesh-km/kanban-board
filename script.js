...
function attachListnersOnATicket(ticketContainer) {
  ticketContainer.addEventListener("click", function (e) {
    const lockUnlock =
      e.currentTarget.querySelector(".lock-unlock").children[0].classList;
    const ticketId = e.currentTarget
      .querySelector(".ticket-id")
      .textContent.trim();
    const ticketContent = ticketContainer.querySelector(".ticket-area");
    const isLockClicked = e.target.classList.contains("fa-solid");
    if (isLockClicked) {
      if (lockUnlock.contains("fa-lock")) {
        lockUnlock.remove("fa-lock");
        lockUnlock.add("fa-unlock");
        ticketContent.setAttribute("contenteditable", true);
      } else {
        lockUnlock.remove("fa-unlock");
        lockUnlock.add("fa-lock");
        saveTicketChangesToDb(ticketId, ticketContent.textContent.trim());
...
function saveTicketChangesToDb(ticketId, textContent) {
  const result = allTicketList.find(({ id }) => id === ticketId);
